...
public class LightningBolt implements IAction
{
    @Override
    public void execute(EntityLivingBase target, @Nullable AbstractMorph morph)
    {
    	Main.log("LightningBolt Executed: " + target.dimension);
        ...
        EntityLightningBolt fireball = new EntityLightningBolt(world, target, d2, d3, d4);
        ...
        if(world.spawnEntity(fireball)) {
        	Main.log("LightningBolt Yay");
        }
        else {
        	Main.log("LightningBolt No");
        }
