...
import com.zeropoints.ensoulomancy.util.Vector3;
    ...
    public static class CameraFlightBuilder {
        ...
        public CameraFlightBuilder addPoint(Vector3 nextPoint, int ticksToFlyThere) {
            if (ticksToFlyThere < 0) {
            	Main.log(Level.WARN, "Tried to add a point with negative tick-timespan to a camera flight. Skipping...");
                return this;
            }
        ...
        public CameraFlight finishAndStart() {
            if(this.flight.flightPoints.size() <= 0) {
                Main.log(Level.WARN, "Tried to start a camera flight without any points! Skipping...");
                return null;
            }
